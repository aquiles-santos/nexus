...
O wrapper Vue (`@nexus/vue` em [`packages/vue`](../../packages/vue/)) só encaminha esse `init` (mais `v-model` e `destroy`). O contrato está em [wrapper-vue.md](./wrapper-vue.md). O `package.json` da raiz **não** lista Vue.
